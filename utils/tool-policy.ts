...
const KNOWN_TOOLS = ["calendar"] as const;
...
export function loadToolAccessConfig(): ToolAccessConfig {
  ...
  if (defaultSection) {
    applySectionRule(rules.calendar, defaultSection, "[tools]", warnings);
  }

  const calendarSection = sections["tool.calendar"] ?? sections.calendar ?? null;
  if (calendarSection) {
    applySectionRule(rules.calendar, calendarSection, "[tool.calendar]", warnings);
  }
...
export function summarizeToolAccess(config: ToolAccessConfig): string {
  const rule = config.tools.calendar;
  const modes: string[] = [];
  if (rule.read) modes.push("read");
  if (rule.write) modes.push("write");
  if (!rule.enabled || modes.length === 0) return "calendar=disabled";
  return `calendar=${modes.join("+")}`;
}
